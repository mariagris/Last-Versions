...
# -*- coding: utf-8 -*-
"""
Created on Tue May 28 13:24:45 2019

@author: mariagris
"""

"""
To be able to se a new frid in each iteration defining a function is the easiest way to do it.
Everything concerning the grid (data, line and bus creation) doesn't change among different iterations.
The net function has followed the same steps as previous files to crete the grid.

After that, the iteration over the json is carried out.
- We run the net once loads are added
- The resulting overloaded lines are attached to a datafrae and later saved in a excel file automatically 
  created.
- It plots each iteration grid and prints the overloaded lines.

At the very end there is a command useful to make a diagnosic of the grid and the problems that 
may arise.
...
def grid():
    ...
    return(net)
"""
#to see and understand the structure, let's have a look at the first iteration
