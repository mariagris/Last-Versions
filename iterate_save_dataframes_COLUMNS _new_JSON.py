...
#import igraph
dfload = pd.read_json(Path(str(os.getcwd()) +'/resultsPFS.json', orient='rows'))
...
for i in dfload['output']:
    ...
    l=[]
    for e in power:
        #assuming their values are in kW as they said:
        pp.create_load(net, bus=e['IdTedisNet'], p_mw = (e['ActivePower'])/1000, q_mvar = (e['ReactivePower'])/1000)
    ...
    for t in range(len(net.res_line['loading_percent'])):
        if net.res_line['loading_percent'][t]>max_percentage:  
    ...
    for e in dataframe_loads['Id Bus']: 
        dataframe_loads[i['timestamp']][e]=(net.load['p_mw'][e],net.load['q_mvar'][e])
